...
5. Confirm `index.html`, `.htaccess`, `llms.txt`, `robots.txt`, `sitemap.xml`, and the `images/` and `documents/` folders are directly inside `public_html`.
...
Static book cover images are included in the `images/` folder and the free sample PDF
is included in the `documents/` folder; both must be uploaded with the ZIP.
